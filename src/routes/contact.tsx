...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { property: "og:url", content: `https://manshi-chauhan.vercel.app/contact` },
    ...
    links: [{ rel: "canonical", href: `https://manshi-chauhan.vercel.app/contact` }],
  }),
  component: Contact,
});
...
const channels = [
  ...
  { Icon: Linkedin, label: "linkedin.com/in/manshi-chauhan", href: "https://www.linkedin.com/in/manshi-chauhan", color: "text-sky" },
...
function Contact() {
  ...
  return (
    <div className="relative">
      <div className="blob bg-clay/30 size-96 -top-10 -left-20 animate-float" />
      <div className="blob bg-plum/25 size-80 top-40 -right-20 animate-float" />
      ...
      <div className="relative mx-auto max-w-4xl px-6 md:px-10 py-20 md:py-28">
        ...
        <Reveal delay={0.15} className="mt-14">
          <form
            onSubmit={onSubmit}
            className="rounded-3xl border border-border bg-card p-6 md:p-10 grid gap-5 relative overflow-hidden"
          >
            <div className="absolute -top-20 -right-20 size-60 rounded-full bg-clay/10 blur-3xl pointer-events-none" />
            ...
            <div className="flex items-center justify-between gap-4 flex-wrap relative">
              ...
              >
                {status === "loading" ? <Loader2 className="size-4 animate-spin" /> : <Send className="size-4 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />}
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {channels.map(({ Icon, label, href, color }) => (
              <a
                ...
                rel="noreferrer"
                className="group flex items-center gap-4 rounded-xl border border-border bg-card p-5 hover:border-clay/60 hover:-translate-y-0.5 transition-all"
              >
                <span className={`grid place-items-center size-10 rounded-full bg-secondary ${color}`}>
                  <Icon className="size-4" />
                </span>
                <span className="text-sm">{label}</span>
                <ArrowUpRight className="size-4 ml-auto text-muted-foreground group-hover:text-clay group-hover:rotate-45 transition-all" />
              </a>
            ))}
          </div>
        </Reveal>
...
function Field({
  ...
  const cls = "w-full rounded-xl border border-border bg-background px-4 py-3 text-sm placeholder:text-muted-foreground focus:outline-none focus:border-clay focus:ring-2 focus:ring-clay/20 transition-all";
  return (
